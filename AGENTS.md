...
- `src/app/components/` — shared UI: `stack-panel/` (the control surface — all actions live here), `layout/` (StatusBar, app shell chrome, nav).
- `src/app/router.tsx` — the route table: lazy imports and `createRoute` definitions.
...
Verify with: `pnpm run check-types`, `npx biome check . --write`, and `pnpm test` (the unit project only, no coverage, under a minute). `pnpm test:integration` covers the suites that spawn a process or a real repo; `pnpm test:all` runs both, and only CI adds `--coverage`.
