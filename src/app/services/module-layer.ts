// 'runtime' needs the machine itself — the filesystem, git, the agent spawn.
// 'corpus' only needs to read/write papercamp/ — the runtime satisfies it, but so
// does a device-local GitHub token (IDEA-193 plan-only). 'client' needs neither.
...
// probe rather than flashing content that's about to 403. A 'corpus' route is
// 'ready' the moment either path into the corpus is available — it never needs to
// wait out the runtime probe once a GitHub token is already configured.
