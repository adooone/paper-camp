...
export const startService = async (name: string): Promise<void> => {
  const response = await fetch(apiUrl(`/api/services/start?name=${encodeURIComponent(name)}`), {
    method: 'POST',
  });
...
export const stopService = async (name: string): Promise<void> => {
  const response = await fetch(apiUrl(`/api/services/stop?name=${encodeURIComponent(name)}`), {
    method: 'POST',
  });
...
export const fetchServiceLog = async (name: string): Promise<string> => {
  const response = await fetch(apiUrl(`/api/services/logs?name=${encodeURIComponent(name)}`));
  if (!response.ok) return '';
