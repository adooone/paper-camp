...
We have 62 tests. Tests are the ultimate durable asset — they guard the code after Fable is gone and let cheaper models safely touch it later. Best targets: `src/app/server/git.ts` (branch-hygiene/merged/ahead-behind logic — the source of recent false-positives), the agent runner (`src/app/server/agent.ts`), and parser round-trips (`src/core/parser.ts`).
...
Give Fable room on UX — but propose-then-build, not silent rewrite. Fable has a *persistent* default frontend house-style that generic nudges won't dislodge, and it over-reaches at high effort, so the boundary matters. Overlaps [[IDEA-34]] (responsive layout / Stack redesign) and the paper-ui 0.5.0 adoption in [[IDEA-32]]/[[IDEA-33]].
...
[[IDEA-25]] as a Fable knowledge-work task — hold the whole system in context and rewrite `papercamp/about.md` to match per-file storage reality.
...
**Touchpoints:** no code changes to enable this — it's a usage pattern. Relies on the existing `model: 'fable'` option (`src/types/index.ts`, `AGENT_OPTIONS`). Tasks 2 and 6 are Fable-run versions of [[IDEA-26]] and [[IDEA-25]]; task 5 overlaps [[IDEA-34]]. If we want these to be repeatable rather than one-off, the reconcile prompt (task 2) is the natural thing to land as the `'reconcile'` TaskKind [[IDEA-26]] proposes.
