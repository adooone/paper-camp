...
- [ ] Make agent rows a fixed two-line card
      Agent rows are unreadable at medium width because their height varies with content. Every row gets the same height at every width: line one the title, line two everything else (agent, time, status), in a denser type size than the current card. `MAX_VISIBLE_TASKS` is already 3 and stays. Remove the `+N more` link added under [[IDEA-163]] — it costs a row of height to say something the Tasks page already answers.
...
- [ ] Reproduce and fix the broken cards at intermediate widths
      Reported directly: Stack panel cards break at some sizes and the panel wastes space again. Not reproduced here — the browser tooling was unavailable when this was written, so this fix starts by finding the widths where it happens rather than assuming. Check the docked band between the three-column breakpoint and large, where the panel is narrowest and card padding, the service rows and the CI card are most likely to overflow or collapse.
