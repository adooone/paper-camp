...
- **One persistence model.** Project name and port require an explicit Save while the agent/model selects save on change; unify on save-on-change with a `Toast` confirmation (after [[IDEA-33]]'s ToastProvider) or Save everywhere.
- **Actions should look like actions.** The Phases-header controls in `plan-detail.tsx` ("Run all phases", "Audit phases against code", "Add as phases") render as plain text labels; give them real button affordances.
...
- **No more `window.confirm`.** Backlog delete in `plans-sidebar.tsx` uses the native confirm dialog; use paper-ui `Modal` (already in 0.2.0) like the rest of the app.
