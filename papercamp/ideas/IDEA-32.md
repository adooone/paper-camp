...
**Shipped via FEAT-34 (in review):** paper-camp is now pinned to `@dendelion/paper-ui: 0.5.0`. The bump brought many new components, a rough.js restyle, and breaking API changes across 0.3–0.5. The breakage list below was measured against actual paper-camp usage and has been handled; it stays as the record of what the migration covered.
...
2. **Alert is now compact single-line (0.5.0).** The `variant` values (`info`/`success`/`warning`/`error`) are retained, but the layout changed. paper-camp had 11 `<Alert>`, several with a `title` + multi-line body (e.g. "Couldn't load plans.md" + error text; the consistency-warnings list). Handled in FEAT-34: terse messages stayed `Alert` with the title folded into the body; rich/multi-line content moved to accent `Card`s (rose/amber) rather than Toast.
