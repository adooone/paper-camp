...
- [ ] Change `POST /api/roadmap/promote` to link instead of delete
      In `src/app/server/routes/content/ideas.ts`, replace the `removeRoadmapItem(...)` call with `linkRoadmapItem(...)` so the item survives and gains the minted id; keep the mint-write-regenerate shape and the 404-on-no-match guard. Decide and document whether a candidate promotion keeps consuming the candidate bullet (queue shape) or also survives-and-links (map shape) — the item itself must stay either way.
- [ ] Resolve items to ideas and roll up progress
