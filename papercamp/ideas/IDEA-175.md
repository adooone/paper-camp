...
- [x] 2026-08-14 [review] [agent] Comments · 1 finding — The restructure into verdict/assessment/concerns/findings, per-destination rendering, the dispatch-with-fallback in postPrReview, the scout-review workflow, and the poller-observes-the-footer SHA recording are all present and well covered by tests, matching the spec. The one substantive risk is that a dispatch GitHub merely accepts (204) is treated as a delivered review even though the workflow may never fire, and with IDEA-173's retry cap deliberately not landed (per the settled clarification), an unobserved review re-dispatches and re-appends a [review] thread line every 60s poll — a spec-acknowledged consequence of shipping phase 5 ahead of IDEA-173.
