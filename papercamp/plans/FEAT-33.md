...
- [ ] Add responsive breakpoints to the root layout
      In router.tsx / the shared paper-ui Layout, stop reserving a fixed paddingRight for the Stack panel — let it overlay content instead of pushing it, below a defined breakpoint (e.g. <1440px). Make the content column flex: 1 with a max-width instead of a hard 1044px fight for space. Below ~1024px, collapse the sidebar into paper-ui's existing (currently unused by paper-camp) mobile drawer mechanics rather than a fixed 220px column.
