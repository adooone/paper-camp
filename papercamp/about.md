...
- `src/app/server/api.ts` — `createApiMiddleware(root)`, a Connect-compatible `(req, res, next)` handler, parsed live from `root`'s `papercamp/`. Shared by both the Vite dev plugin (`pnpm dev`) and `dev-server.ts` (`paper-camp dev`). It wires up the server-side managers — `activity.ts` (fs-watch SSE), `git.ts` (working-tree/branch operations), `status.ts` (lint/format/test check runner), `agent.ts` + `agent-hooks.ts` (agent task orchestration: per-phase commits, audit stamping) — and dispatches to per-resource route modules under `src/app/server/routes/`. `/api/plans` and `/api/ideas` remain two separate routes rather than collapsing into one `/api/entities`, but both are just filtered views over the same `papercamp/ideas/` corpus:
  ...
  - `routes/status.ts` — `GET /api/status` (lint/format/test check results), `POST /api/status/check` (run one), `POST /api/status/fix` (biome --write), and `GET /api/activity/stream` — an SSE endpoint backed by `activity.ts`, which recursively watches `papercamp/` (per-file trees, archive, and the monolithic files alike) and pushes a debounced tick. Nine client modules subscribe (the Stack panel, the git page, and the `use-*-client` hooks), each routing on `payload.type` and refetching only its own slice. It deliberately does not synthesize a human-readable activity feed anymore.
...
- `src/app/router.tsx` — code-based TanStack Router tree: one root route rendering paper-ui's `Layout` (header, sidebar, and automatic page-wrap all off; a `navigationIsland` slot holding `ProjectIdentityHeader`, a docs-search `Input` shown only on `/docs`, and ghost `Button`s for the four nav items) + a manually-wrapped `Page` around `Outlet`, plus a persistent `StackPanel` (pinned open above 1440px, where the root wrapper's `min-[1440px]:pr-[480px]` reserves its width; an overlay drawer below that). Four child routes: Plans (`/`), Review (`/review`), Docs (`/docs`), Settings (`/settings`). Each route's sidebar slot renders `PlanFilterColumn` + `PlanActionsColumn` (Plans), `ReviewSidebar` + `PlanActionsColumn` (Review), or the Docs/Settings sidebars — `PlanActionsColumn` swaps in for the filters while an entity is open.
...
- `src/app/components/stack-panel/` — see "The Stack panel" under Dashboard below.
...
**The Stack panel** (`src/app/components/stack-panel/`) is not a route — it's a fixed, right-docked, full-height panel mounted once in `router.tsx`, rendered *after* the content scroll container closes so it never scrolls away (guarded by `chrome-outside-scroll.guard.test.ts`). Present on every page. Width `min(480px, 100vw)`; chalkboard-textured, desk-green gradient background, Luminari header. Default **closed**, with the open state persisted to `localStorage` under `stack-open`. At ≥1440px (`LARGE_SCREEN_QUERY`) it is `pinned`: permanently open with the close button and reopen tab both hidden, and the root wrapper reserves its width with `min-[1440px]:pr-[480px]` so it doesn't overlay content. Below that it's an overlay drawer toggled by a chalkboard tab on the right edge — the tab shows a `Spinner` while an agent is active, a rose dot when checks are failing, else "S". Open/close is a bare `transform` with no transition; the framer-motion slide was removed by IDEA-152. Two sections, Agent (fixed height) over Desk:
- **Agent** (`agent-section.tsx`) — up to `MAX_VISIBLE_TASKS` (3) tasks from `/api/agent/status`, in a container holding a reserved `basis-[9.25rem]` so the empty state doesn't shrink the panel. Each task is a chalkboard `Card`: plan title + task kind (phase N / audit / batch reconcile / drafting / extending / suggesting commit message / syncing / run all) + agent label, a status `Stamp` (starting/running/stopping/done/error/superseded), and a stop `IconButton` while running. The card opens `/tasks?taskId=…`, where the streamed output lives; an `errorKind: 'auth'` task instead stamps "stopped — agent signed out", linking to Settings → Setup. "No agent running." when idle.
- **Desk** (`desk-section.tsx`) — rendered only when `desk` exists in `papercamp/config.json`, in four groups:
  - **Services** (`services-group.tsx`, `/api/services`) — one `Card` per `desk.services` entry: a health/status dot, name, port, a start/stop `IconButton`, and a log `<pre>` (`/api/services/logs`, polled every 1.5s while running) toggled by clicking the name.
  - **Checks** (`checks-group.tsx`, `/api/checks`) — one clickable `Stamp` per `desk.checks` entry, each running its command via `/api/checks/run`; the first failing check adds a `CopyButton` carrying a fix prompt built from its captured output.
  - **Build** (`build-group.tsx`) — reads `status.build` (backed by `commands.build`, IDEA-158) rather than `/api/checks`: a clickable `build` stamp plus a last-built time, or a message when no command is configured.
  - **CI & release** (`ci-group.tsx`, `/api/ci`) — recent workflow runs for `desk.ci.branch` with status dots and links, the released and queued versions, and links to the release PR and the repo's Actions tab.

The git commit/push/sync flow is **not** in this panel — it lives on the `/git` route and the `StatusBar` (see the surface split under IDEA-146).

The panel subscribes to `GET /api/activity/stream` (SSE), routing ticks by `payload.type`: a `status` tick refreshes the check status (80ms debounce), an `agent` tick the agent tasks (120ms), and a generic `changed` tick reloads plans/suggestions/status/consistency/doctor/git/agent/archivable-ideas (250ms) — one timer per event type so a chatty agent log can't starve a pending check refresh. It also refreshes plans and git status on `window` focus, to catch remote changes faster than the server's own poll. There is no rendered activity feed.
...
- Stack panel: agent task cards with stop control over a Desk section (services with logs, config-driven checks with a copyable fix prompt, build, CI & release), refreshed live off the SSE change stream, present on every page
